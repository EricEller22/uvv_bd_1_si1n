...
CREATE DATABASE uvv
COMMENT 'Banco de dados utilizado para a implementação do banco de dados HR proposto no Pset1 ';
   
   
-- String para conceder privilégios no banco de dados 'uvv' ao usuário 'eric'
...
-- String para criar a tabela de cargos e seus respectivos comentarios
CREATE TABLE cargos (
                id_cargo VARCHAR(10) NOT NULL COMMENT 'Primary key(PK) que identifica um cargo.',
                cargo VARCHAR(35) NOT NULL COMMENT 'Nome do cargo',
                salario_minimo NUMERIC(8,2) COMMENT 'Menor salário de um cargo',
                salario_maximo NUMERIC(8,2) COMMENT 'Maior salário de um cargo.',
                CONSTRAINT id_cargo_pk PRIMARY KEY (id_cargo)
)COMMENT 'Tabela que armazena os cargos existentes na empresa, e seus respectivos salários (Minimo) e (Máximo).';
...
-- String para criar a tabela de regioes e seus respectivos comentarios
CREATE TABLE regioes (
                Id_regiao INTEGER NOT NULL COMMENT 'Primary key(PK) que identifica uma região especifica dentro dessa tabela.',
                nome VARCHAR(25) NOT NULL COMMENT 'Indice único que identifica o nome da região',
                CONSTRAINT id_regiao_pk PRIMARY KEY (Id_regiao)
)COMMENT 'Tabela de regiões. Dentro desta tabela você encontra o nome da região e seu respectivo identificador(id).';
...
-- String para criar a tabela paises e seus respectivos comentarios
CREATE TABLE paises (
                id_pais CHAR(2) NOT NULL COMMENT 'Primary key(PK) que identifica um país especifico dentro dessa tabela.',
                nome VARCHAR(50) NOT NULL COMMENT 'indice único que identifica o nome do país',
                Id_regiao INTEGER NOT NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela regioes',       
                CONSTRAINT id_pais_pk PRIMARY KEY (id_pais)
)COMMENT 'Tabela que armazena dados sobre os paises, como por exemplo a regiao em que ele se encontra, seu respectivo nome e seu pŕoprio identificador(id)';
...
-- String para criar a tabela de localizacoes e seus respectivos comentarios
CREATE TABLE localizacoes (
                id_localizacao INTEGER NOT NULL COMMENT 'Primary key(PK) que identifica uma localização dentro da tabela.',
                endereco VARCHAR(50)COMMENT 'identifica o endereço de um departamento ou escritório da empresa.',
                cep VARCHAR(12)COMMENT 'Identifica o CEP localizador de um departamento ou escritório da empresa.',
                cidade VARCHAR(50)COMMENT 'Identifica a cidade em que está localizado um departamento ou escritório da empresa.',
                uf VARCHAR(25)COMMENT 'Identifica o estado onde é localizado um departamento ou escritório da empresa.',
                id_pais CHAR(2) NOT NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela de paises.',
                CONSTRAINT id_localizacao_pk PRIMARY KEY (id_localizacao)
)COMMENT 'Tabela de localições armazena dados sobre a localização dos diversos departamentos. Como por exemplo seu endereço, cidade e estado.';



-- String para criar a tabela de departamentos e seus respectivos comentarios
CREATE TABLE departamentos (
                id_departamento INTEGER NOT NULL COMMENT 'Primary key(PK) que identifica um departamento dentro da tabela.',
                nome VARCHAR(50)COMMENT 'Identifica o nome de um departamento.',
                id_localizacao INTEGER NOT NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela de localização. E identifica onde é a localização do departamento.',
                id_gerente INTEGER NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela de empregados. E identifica o gerente responsável pelo departamento.',
                CONSTRAINT id_departamento_pk PRIMARY KEY (id_departamento)
)COMMENT 'Tabela departamento. Reponsável por identificar os departamentos da empresa, informando sua localização e qual o seu supervisor';


-- String para criar a tabela de empregados e seus respectivos comentarios
CREATE TABLE empregados (
                id_empregado INTEGER NOT NULL COMMENT 'Primary key(PK) que identifica um empregado dentro da tabela.',
                nome VARCHAR(75) NOT NULL COMMENT 'Referente ao nome completo do empregado',
                email VARCHAR(35) NOT NULL COMMENT 'Referente ao email inicial (Antes do @) do empregado.',
                telefone VARCHAR(20)COMMENT 'Referente ao telefone completo do empregado, incluindo o código do seu respectivo pais e estado.',
                data_contratacao DATE NOT NULL COMMENT 'Referente a data em que o empregado iniciou no cargo em que ele está.',
                id_cargo VARCHAR(10) NOT NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela de cargos. E identifica qual o cargo que esta sendo oculpado pelo empregado.',
                salario NUMERIC(8,2)COMMENT 'Referente ao salário mensal do empregado.',
                comissao NUMERIC(4,2)COMMENT 'Referente a porcentagem de comissão de um empregado. no entanto, apenas empregados alocados no departamento de vendas recebem comissões.',
                id_departamento INTEGER NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela de departamentos. E identifica em qual departamento aquele empregado está alocado.',
                id_supervisor INTEGER NULL COMMENT 'Chave estrangeira (FK), que faz referência à própria tabela de empregados, e identifica o supervisor direto de um empregado.',
                CONSTRAINT id_empregado_pk PRIMARY KEY (id_empregado)
)COMMENT 'Tabela responável por informar todos os dados de um empregado da empresa.';
...
-- String para criar a tabela de historico de cargos e seus respectivos comentarios
CREATE TABLE historico_cargos (
                id_empregado INTEGER NOT NULL COMMENT 'Primary key(PK) que identifica um empregado dentro da tabela. Mas que também é uma Foreign Key(Fk) que faz referência à Primary Key(PK) da tabela empregados.',
                data_inicial DATE NOT NULL COMMENT 'Primary key(PK) que identifica a data que um empregado engressou em um novo cargo dentro da empresa.',
                data_final DATE NOT NULL COMMENT 'Identifica a data em que um empregado deixou de fazer parte de um cargo.',
                id_cargo VARCHAR(10) NOT NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela de cargos. E identifica qual o cargo que estava sendo oculpado por um empregado, antes deste deixar de fazer parte.',
                id_departamento INTEGER NOT NULL COMMENT 'Chave estrangeira (FK), que faz referência à tabela de departamentos. E identifica qual o departamento que o empregado estava alocado, antes deste deixar de fazer parte.',
                CONSTRAINT id_empregado_pfk PRIMARY KEY (id_empregado, data_inicial)
)COMMENT 'Tabela que armazena todo o histórico dos cargos pelo qual um empregado já passou. Observa-se que, se um empregado mudar de cargo dentro de um departamento ou vice-versa, as informações antigas teram que ficar guardadas dentro desta tabela.';
